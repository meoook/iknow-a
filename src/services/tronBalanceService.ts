import { IFinanceToken } from '../types';
import { fetchWith429Retry } from './httpUtils';

/**
 * Normalizes Tron RPC URLs (converts wss/ws to https/http)
 */
export function normalizeTronRpcUrl(rpcUrl: string): string {
  if (!rpcUrl) return 'https://api.trongrid.io';
  const trimmed = rpcUrl.trim();
  if (trimmed.startsWith('wss://')) {
    return trimmed.replace(/^wss:\/\//, 'https://');
  }
  if (trimmed.startsWith('ws://')) {
    return trimmed.replace(/^ws:\/\//, 'http://');
  }
  return trimmed.replace(/\/+$/, '');
}

const ALPHABET = '123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz';

/**
 * Decodes a Base58 string into Uint8Array
 */
function base58Decode(str: string): Uint8Array {
  const bytes = [0];
  for (let i = 0; i < str.length; i++) {
    const char = str[i];
    const val = ALPHABET.indexOf(char);
    if (val === -1) throw new Error(`Недопустимый символ Base58: ${char}`);
    for (let j = 0; j < bytes.length; j++) {
      bytes[j] *= 58;
    }
    bytes[0] += val;
    let carry = 0;
    for (let j = 0; j < bytes.length; j++) {
      bytes[j] += carry;
      carry = bytes[j] >> 8;
      bytes[j] &= 0xff;
    }
    while (carry > 0) {
      bytes.push(carry & 0xff);
      carry >>= 8;
    }
  }
  for (let i = 0; i < str.length && str[i] === '1'; i++) {
    bytes.push(0);
  }
  return new Uint8Array(bytes.reverse());
}

/**
 * Encodes a Tron Base58 address into a 32-byte (64 hex characters) ABI parameter
 */
function encodeAddressParameter(base58Address: string): string {
  const decoded = base58Decode(base58Address);
  if (decoded.length !== 25) {
    throw new Error(`Некорректный адрес Tron: ${base58Address}`);
  }
  // Tron addresses: 1 byte prefix (0x41) + 20 bytes address payload + 4 bytes checksum
  const addressPayload = decoded.slice(1, 21);
  const hex = Array.from(addressPayload)
    .map((b) => b.toString(16).padStart(2, '0'))
    .join('');
  return hex.padStart(64, '0');
}

/**
 * Formats a BigInt raw balance into a string given decimals
 */
function formatUnits(balance: bigint, decimals: number): string {
  if (balance === 0n) return '0';
  const factor = 10n ** BigInt(decimals);
  const intPart = balance / factor;
  const remPart = balance % factor;

  if (remPart === 0n) return intPart.toString();
  const remStr = remPart.toString().padStart(decimals, '0').replace(/0+$/, '');
  return `${intPart}.${remStr}`;
}

/**
 * Fetches native TRX balance using TronGrid v1 or Tron fullnode JSON RPC
 */
export async function fetchTronNativeBalance(
  rpcUrl: string,
  walletAddress: string,
  decimals = 6
): Promise<string> {
  const url = normalizeTronRpcUrl(rpcUrl);

  try {
    const res = await fetchWith429Retry(`${url}/v1/accounts/${walletAddress}`, {
      method: 'GET',
      headers: { Accept: 'application/json' },
    });

    if (res.ok) {
      const json = await res.json();
      if (json.data && json.data.length > 0) {
        const rawSun = BigInt(json.data[0].balance || 0);
        return formatUnits(rawSun, decimals);
      }
      return '0';
    }
  } catch {
    // Fallback to /wallet/getaccount
  }

  // Fallback to /wallet/getaccount
  const resFallback = await fetchWith429Retry(`${url}/wallet/getaccount`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ address: walletAddress, visible: true }),
  });

  if (!resFallback.ok) throw new Error(`Tron RPC error: ${resFallback.status}`);

  const json = await resFallback.json();
  const rawSun = BigInt(json.balance || 0);
  return formatUnits(rawSun, decimals);
}

/**
 * Fetches TRC-20 token balance via triggerconstantcontract balanceOf
 */
export async function fetchTronTrc20Balance(
  rpcUrl: string,
  walletAddress: string,
  tokenAddress: string,
  decimals = 6
): Promise<string> {
  const url = normalizeTronRpcUrl(rpcUrl);
  const parameter = encodeAddressParameter(walletAddress);

  const res = await fetchWith429Retry(`${url}/wallet/triggerconstantcontract`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      owner_address: walletAddress,
      contract_address: tokenAddress,
      function_selector: 'balanceOf(address)',
      parameter,
      visible: true,
    }),
  });

  if (!res.ok) throw new Error(`Tron TRC-20 RPC error: ${res.status}`);

  const json = await res.json();
  if (json.result && json.result.result === false) {
    const msg = json.result.message ? Buffer.from(json.result.message, 'hex').toString('utf8') : 'Ошибка контракта';
    throw new Error(msg || 'Tron contract error');
  }

  const hexResult = json.constant_result?.[0];
  if (!hexResult) return '0';

  const rawBalance = BigInt(`0x${hexResult}`);
  return formatUnits(rawBalance, decimals);
}

export interface TronChainBalanceResult {
  native: string | null;
  tokens: Record<number, string | null>;
  nativeError?: string | null;
  tokenErrors?: Record<number, string>;
}

/**
 * Fetches all balances for a Tron chain (attempts v1 batch fetch first, falls back to direct queries)
 */
export async function fetchTronBalances(
  rpcUrl: string,
  walletAddress: string,
  nativeDecimals = 6,
  tokens: IFinanceToken[] = []
): Promise<TronChainBalanceResult> {
  const url = normalizeTronRpcUrl(rpcUrl);
  const result: TronChainBalanceResult = {
    native: null,
    tokens: {},
    tokenErrors: {},
  };

  let v1Success = false;

  // 1. Try TronGrid v1 account endpoint which returns native and all TRC-20 in one request
  try {
    const res = await fetchWith429Retry(`${url}/v1/accounts/${walletAddress}`, {
      method: 'GET',
      headers: { Accept: 'application/json' },
    });

    if (res.ok) {
      const json = await res.json();
      if (json.data && json.data.length > 0) {
        const acc = json.data[0];
        result.native = formatUnits(BigInt(acc.balance || 0), nativeDecimals);

        // Parse TRC-20 tokens map
        const trc20List: Record<string, string>[] = acc.trc20 || [];
        const tokenMap = new Map<string, string>();
        trc20List.forEach((item) => {
          Object.entries(item).forEach(([contract, amount]) => {
            tokenMap.set(contract.toLowerCase(), amount);
          });
        });

        tokens.forEach((token) => {
          const rawAmount = tokenMap.get(token.address.toLowerCase());
          if (rawAmount !== undefined) {
            result.tokens[token.id] = formatUnits(BigInt(rawAmount), token.decimals ?? 6);
          } else {
            result.tokens[token.id] = '0';
          }
        });

        v1Success = true;
      } else if (json.data && json.data.length === 0) {
        // Account exists with 0 balance
        result.native = '0';
        tokens.forEach((t) => {
          result.tokens[t.id] = '0';
        });
        v1Success = true;
      }
    }
  } catch {
    // Proceed to fallback
  }

  if (v1Success) return result;

  // 2. Fallback to individual calls
  try {
    result.native = await fetchTronNativeBalance(rpcUrl, walletAddress, nativeDecimals);
  } catch (err: any) {
    result.nativeError = err?.message || 'Ошибка RPC Tron';
  }

  if (tokens.length > 0) {
    const tokenPromises = tokens.map(async (token) => {
      try {
        const bal = await fetchTronTrc20Balance(
          rpcUrl,
          walletAddress,
          token.address,
          token.decimals ?? 6
        );
        result.tokens[token.id] = bal;
      } catch (err: any) {
        result.tokens[token.id] = null;
        if (result.tokenErrors) {
          result.tokenErrors[token.id] = err?.message || 'Ошибка TRC-20';
        }
      }
    });

    await Promise.allSettled(tokenPromises);
  }

  return result;
}
